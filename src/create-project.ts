...
import { copyTemplateFiles, createError, databaseSetup, gqlTracker, restTracker } from './utils'
...
export const createProject = async (options: Partial<IMainOptions>) => {
  ...
  try {
    ...
    Logger.error(`${chalk.red.bold('ERROR')}, Invalid template name, ${message}`)
    process.exit(1)
  }
  ...
  const tasks = new Listr([
    ...
    },
    {
      ...
      task: () => (options.implementation === 'gql' ? gqlTracker : restTracker).init(options as IMainOptions),
    },
  ])
