import { getKeysAndTypes } from '../../../../../cli/v2/builder/actions/custom-type/shared/get-keys-and-types'
import Logger from '../../../../../logger/logger'
import { ICustomTypeCreationParams, ICustomTypeProp, InterfaceStorageType } from '../../../../../types'
import {
  RestProjectTracker,
  GqlProjectTracker,
...
const checkForSpecialChars = (
  ...
  return errors.length ? { isValid: ValidationRes.INVALID, message: errors } : { isValid: ValidationRes.VALID }
}
  ...
  if (newPropKey)
    return keys.includes(newPropKey)
      ? {
          ...
          message: 'Type property name already exists - no duplicate type props allowed.',
        }
...
export const validateCustomTypeName = async (
  ...
  if (!typeName || typeName.length === 0 || specialCharsValid.isValid !== ValidationRes.VALID)
    return {
      isValid: ValidationRes.INVALID,
...
export const validateCustomTypeProp = (
  ...
  const currentTypeProps: string[] | undefined = tracker.getFromStorage(InterfaceStorageType.typeCreationProps)
  ...
  if (!typeName || specialCharsValidForTypeName.isValid !== ValidationRes.VALID) {
    return {
      isValid: ValidationRes.INVALID,
