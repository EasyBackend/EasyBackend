...
export const confirmWithUserAndNavigate = async (
  ...
  const confirmType = tracker.getFromStorage(InterfaceStorageType.confirmTypeCreation)
...
export const confirmTypeCreationWithUser = async (tracker: RestProjectTracker | GqlProjectTracker) => {
  printCustomTypeDetails(tracker)

  const { confirmType } = await inquirer.prompt([customTypeQuestions.confirmType])

  tracker.addToStorage({ key: InterfaceStorageType.confirmTypeCreation, value: confirmType }, true)
}
