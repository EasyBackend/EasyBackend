...
export const printCustomTypeDetails = (tracker: RestProjectTracker | GqlProjectTracker) => {
  const typeName = tracker.getFromStorage(InterfaceStorageType.typeName)

  const typeProperties = tracker.getFromStorage(InterfaceStorageType.typeCreationProps)

  tracker.writeToBottomBar(
    `${chalk.green('Type name: ')}${typeName}\n\n${chalk.yellow(' Properties: ')}${typeProperties.join(', ')}`,
    true
  )
}

export const logAllValidTypes = (tracker: RestProjectTracker | GqlProjectTracker, typeProperties: string[]) => {
  ...
  tracker.writeToBottomBar(
    ...
    ${customTypes.length ? chalk.yellow('\nCustom types: ', customTypes.join(', ')) : ''}
    ${
